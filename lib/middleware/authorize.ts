...
import { AuthenticatedRequest } from './enhanced-auth.js';
...
export function withAdminRole(handler: AuthorizedHandler) {
  return withRoles(['admin'])(handler);
}

export function withSelfOrAdmin(getUserId: (req: AuthenticatedRequest) => number | string) {
  return function (handler: AuthorizedHandler) {
    return async (req: AuthenticatedRequest, res: VercelResponse) => {
...
export function preventSelfDemotion(handler: AuthorizedHandler) {
  return async (req: AuthenticatedRequest, res: VercelResponse) => {
