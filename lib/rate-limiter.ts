...
interface RateLimitConfig {
  ...
  blockDurationMs: number; // Base block duration for exponential backoff
}
 ...
 */
function calculateBlockDuration(failureCount: number, baseMs: number): number {
 ...
 */
export function createAuthRateLimit(config: RateLimitConfig = LOGIN_CONFIG) {
 ...
 */
export function recordAuthFailure(req: VercelRequest, email?: string): void {
  const clientIP = getClientIP(req);
  const ipKey = `auth:ip:${clientIP}`;
 ...
 */
export function recordAuthSuccess(req: VercelRequest, email: string): void {
  const clientIP = getClientIP(req);
  const ipKey = `auth:ip:${clientIP}`;
  const emailKey = `auth:email:${email.toLowerCase().trim()}`;
