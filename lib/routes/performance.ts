...
import { AuthenticatedRequest, requireAuth, setCORSHeaders, setSecurityHeaders } from '../auth.js';
  ...
  try {
    ...
    if (format === 'prometheus') {
      return res
        ...
        .send(formatPrometheusMetrics(result));
    }
...
function formatPrometheusMetrics(data: any): string {
  ...
  if (data.basic) {
    ...
    metrics.push(
      `database_connection_pool_usage_percent ${data.basic.connectionPool.usage} ${timestamp}`
    );
    ...
    const statusValue =
      data.basic.status === 'healthy' ? 0 : data.basic.status === 'warning' ? 1 : 2;
  ...
  if (data.performanceTests) {
    metrics.push(
      `# HELP database_simple_query_time_ms Simple query execution time in milliseconds`
    );
    metrics.push(`# TYPE database_simple_query_time_ms gauge`);
    metrics.push(
      `database_simple_query_time_ms ${data.performanceTests.testResults.simpleQuery} ${timestamp}`
    );

    metrics.push(
      `# HELP database_complex_query_time_ms Complex query execution time in milliseconds`
    );
    metrics.push(`# TYPE database_complex_query_time_ms gauge`);
    metrics.push(
      `database_complex_query_time_ms ${data.performanceTests.testResults.complexQuery} ${timestamp}`
    );
    ...
    metrics.push(
      `database_insert_time_ms ${data.performanceTests.testResults.insertTest} ${timestamp}`
    );
  }
