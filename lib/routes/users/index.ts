...
import { logUserCreation } from '../../audit-logger.js';
...
import { createUserSchema, getUsersSchema } from '../../schemas/user.js';

// GET /api/users - List users (authenticated users can read)
const getUsersHandler = withCORS(
  withErrorHandling(
    withAuth(
      validateQuery(getUsersSchema)(async (req: AuthenticatedRequest, res: VercelResponse) => {
...
const createUserHandler = withCORS(
  withErrorHandling(
    withAuth(
      withAdminRole(
        validateBody(createUserSchema)(async (req: AuthenticatedRequest, res: VercelResponse) => {
...
async function handleGetUsers(req: AuthenticatedRequest, res: VercelResponse): Promise<void> {
  ...
  const { page, size, search, active, role, createdFrom, createdTo, sort, order } =
    req.query as any;
