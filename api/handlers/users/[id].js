...
import { preventSelfDemotion, validateBody, validateQuery, withAdminRole, withAuth, withCORS, withErrorHandling, withSelfOrAdmin, } from '../../lib/middleware/index.js';
import prisma from '../../lib/prisma.js';
import { updateUserSchema, userIdSchema } from '../../lib/schemas/user.js';
// GET /api/users/[id] - Get user by ID (self or admin)
const getUserHandler = withCORS(withErrorHandling(withAuth(withSelfOrAdmin((req) => req.query.id)(validateQuery(userIdSchema)(async (req, res) => {
    const userId = Number(req.query.id);
    await handleGetUser(req, res, userId);
})))));
// PUT /api/users/[id] - Update user (self or admin, with admin protections)
const updateUserHandler = withCORS(withErrorHandling(withAuth(withSelfOrAdmin((req) => req.query.id)(preventSelfDemotion(validateQuery(userIdSchema)(validateBody(updateUserSchema)(async (req, res) => {
    const userId = Number(req.query.id);
    await handleUpdateUser(req, res, userId);
})))))));
// DELETE /api/users/[id] - Soft delete user (admin only, cannot delete self)
const deleteUserHandler = withCORS(withErrorHandling(withAuth(withAdminRole(validateQuery(userIdSchema)(async (req, res) => {
    const userId = Number(req.query.id);
...
const restoreUserHandler = withCORS(withErrorHandling(withAuth(withAdminRole(validateQuery(userIdSchema)(async (req, res) => {
    const userId = Number(req.query.id);
